...
- Motors: all three (mouth/head/tail) wired to 2× DRV8833 breakouts and bench-verified. `hal.c`
  ...
  track duty proportionally (barely moves below ~65%, needs ~100% for a full stroke — see
  `WIRING.md` §6.1), so mouth lip-sync is a binary open/close gate off a smoothed RMS envelope of
  the audio actually being played, not a proportional value. The tail flaps once on any
