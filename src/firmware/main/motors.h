...
// Activation-boundary recovery: no-op if nFAULT isn't latched. Otherwise requires nFAULT high,
// wakes the drivers with all duties zero, waits for the datasheet's 1 ms startup time, and
// requires nFAULT to remain high. Leaves the drivers parked whether recovery succeeds or fails.
